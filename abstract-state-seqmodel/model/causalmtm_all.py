...
class CausalTransformer(nn.Module):
    ...
    def causal_attn_mask(self, ctx_size: int, device: Device):
        max_len = self.max_goal_length + ctx_size * 2
        ...
        # observations attend to current/previous obs and goal
        obs_to_goal = torch.zeros(ctx_size, self.max_goal_length)
        obs_to_obs = torch.triu(torch.ones(ctx_size, ctx_size), diagonal=1)
        obs_to_obs = obs_to_obs.masked_fill(obs_to_obs == 1., float("-inf"))
        obs_to_actions = torch.ones(ctx_size, ctx_size) * float("-inf")
        obs_to_all = torch.cat([obs_to_goal, obs_to_obs, obs_to_actions], dim=1)

        # actions attend to current/previous actions, obs and goal
        action_to_goal = torch.zeros(ctx_size, self.max_goal_length)
        action_to_obs = torch.triu(torch.ones(ctx_size, ctx_size), diagonal=1)
        action_to_obs = action_to_obs.masked_fill(action_to_obs == 1., float("-inf"))
    ...
    def forward(self, observations: Tensor, actions: Tensor, goals: Tensor, padding_mask: Tensor, probe_layer=None):
        batch_size, ctx_size = observations.size()[:2]
        ...
        seq_padding_mask = torch.cat([goal_padding_mask, padding_mask, padding_mask], dim=1)  # TODO: pad goal seqs
        ...
        pred_a = outputs[:, self.max_goal_length + ctx_size:]
...
class CausalforMTM(pl.LightningModule):
    ...
    def ar_step_end(self, timestep: int, next_obs: Tensor, action: Tensor, obs_seq: Tensor, action_seq: Tensor):
        ...
        next_obs = next_obs.view(1, 1, 3, self.obs_height, self.obs_height)
